...
\chapter{Wprowadzenie}
    ...
    g³ówn± domen± jest sztuczna inteligencja, w szczególno¶ci jej dzia³, po¶wiêcony inteligentnym
...
\begin{itemize}
    ...
    \item Komunikaty wysy³ane przez Sêdziego maj± nastêpuj±cy format: zaczynaj± siê od nawiasów kwadratowych,
          ...
          która zostanie przekazana odpowiednim botom. Komunikat musi koñczy siê znakiem
          nowej linii, n.p: '[1,2,3,4,5]INIT'. Komunikaty musz± byæ jednolinijkowe i nie mog± zawieraæ ¿adnych znaków spoza 7-bitowego kodowania
          ASCII. 
    \item Sêdzia otrzymuje komunikaty zwrotne od botów w takiej kolejno¶ci w jakiej zostali wylistowani.
    \item Komunikaty od nadzorcy do botów i do Sêdziego równie¿ koñcz± siê znakiem nowej linii.
    ...
    \item Ponadto, je¶li Sêdzia stwierdzi³, ¿e gra siê zakoñczy³a i chce poinformowaæ wszystkich o tym,
          ¿e nast±pi³ koniec gry wysy³a do Nadzorcy komunikat o tre¶ci "[0]END".
          Nastêpnie powinien wys³aæ komunikat zawieraj±cy punktacjê dla wszystkich graczy w postaci: '[score1, score2, ... ]'.
    \item Je¶li Sêdzia chce zakoñczyæ dzia³anie którego¶ z graczy nale¿y wys³aæ do niego wiadomo¶æ KILL (np. "[4]KILL").
          Mo¿na te¿ w ten sposób zabiæ wiêksz± liczbê graczy lub nawet wszystkich ("[0]KILL")
    \item Je¶li Sêdzia wy¶le komunikat do Bota którego dzia³anie zosta³o przerwane (czy to na skutek wewnêtrznego b³êdu, czy przez zabicie przez Sêdziego), 
          dostanie komunikat zwrotny '\_DEAD\_' (wys³any przez Nadzorcê). Boty powinny unikaæ wysy³ania tego komunikatu, gdy¿ mog± zostaæ uznane 
          przez Sêdziego za martwe.
\end{itemize}
...
Innym z zadañ nadzorcy jest odczytywanie informacji z standardowego strumienia b³êdów Botów i Sêdziego, a nastêpnie przekazanie ich warstwie Schedulera. 
Pozwala to na umo¿liwienie u¿ytkownikowi dostêp do informacji które jego program wypisywa³ na stderr. 
Funkcjonalno¶æ ta jest realizowana przez Nadzorcê, proprzez stworzenie dodatkowego w±teu odpowiedzialnego za czytanie infromacji 
pojawiaj±cych siê na strumieniu b³êdów Sêdziego i Botów. 
W±tek ten przy u¿yciu polecenia select czeka na mo¿liwo¶æ nieblokuj±cego odczytu na odpowiednich deskryptorach, 
a gdy taka mo¿liwo¶æ siê pojawia odczytuje informacje a¿ do opró¿nienia potoku zwi±zanego z danym deskryptorem.

Nietrywialnym problemem jest wybór mechanizmu odpowiedzialnego za ograniczanie i zbieranie informacji o zu¿ywanych przez Sêdziego i boty zasobach systemowych. 
Dostêpnych metod jest wiele, jednak nie wszystkie s± mo¿liwe do zastosowania w przypadku systemu AI-Arena. 
Mechanizm ten, jak równie¿ motywacje do jego wyboru zosta³y przedstawione w rozdziale 8.1.
...
Ka¿da aplikacja korzystaj±ca z tego frameworku sk³ada siê z trzech czê¶ci: klienta, workera oraz serwera. Serwer jest dostarczany przez
aplikacjê Gearmand (v. 0.26). W naszym przypadku rolê klienta pe³ni czê¶æ webowa serwisu zlecaj±ca serwerowi rozegranie odpowiedniej Rozgrywki, 
podaj±c jako argumenty grê, boty i Sêdziego Rozgrywki. Nastêpnie serwer wysy³a ¿±danie do tzw. "workera" zajmuj±cego siê wy³±cznie wykonywaniem zleceñ.
Workerów mo¿e byæ wiele i mog± dzia³aæ na fizycznie ró¿nych maszynach. Problemem równomiernego obci±¿enia workerów zamuje siê serwer. 
W przypadku AI-Areny funkcjonalno¶æ workera ogranicza siê do uruchomienia nadzorcy z odpowiednimi parametrami, ¿eby po zakoñczeniu Rozgrywki
...
Umo¿liwia tak¿e u¿ycie wielu klientów, serwerów i workerów tworz±cych klaster. Nale¿y dodaæ, i¿ w przypadku faktycznego wykorzystania wielu serwerów i workerów
ca³kowita kontrolê nad nimi sprawuje aplikacja Gearman. Dba ona miêdzy innymi o równomierne wykorzystanie zasobów. U¿ycie zapasowego serwera
...
\section{Serwis webowy}
    ...
    Naturalnie wykorzystane zosta³u równie¿ arkusze stylów CSS. Dodatkowo zosta³y wykorzystane pewne standardowe programy linuxowe, jak
    np. Makefile.
...
jest PostgreSQL. Tabele w bazie danych s± tworzone przez Django w sposób automatyczny
...
podczas uruchomienia automatycznych testów. W przypadku gdy dana metoda korzysta z bazy danych, nie u¿ywa tej "produkcyjnej",
...
Inny mechanizm systemowy który by³ rozwa¿any jako rozwi±zanie tych problemów, to grupy procesów. Mechanizm ten pozwal³by miêdzy innymi na wys³anie sygna³u do procesu i wszystkich jego potomków, jak równie¿ prostsze monitorowanie zu¿ycia zasobów systmowych przez grupê procesów. Niestety poniewa¿ tego samego mechanizmu u¿ywa pow³oka systemowa, stwarza to z³o¶liwemu u¿ytkownikowi mo¿liwo¶æ tworzenia procesów, które zostan± uznane za procesy pow³oki i uwolnienia siê tym samym spod kontroli.\\
...
Autorzy systemu rozwazali trzy ró¿ne rozwi±zania pierwszego podproblemu: podmienianie bibliotek, skanowanie kodu i systrace.\\
