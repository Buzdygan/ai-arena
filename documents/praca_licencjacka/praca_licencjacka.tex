...
\chapter{Metodologia Pracy}
    ...
    miêdzy kolejnymi spotkaniami na zajêciach z prowadz±cym.\\
    ...
    Z uwagi na ma³± liczno¶æ zespo³u zdecydowali¶my siê nie u¿ywac narzêdzi wspomagaj±cych ¶ledzenie zmian. Do obowi±zków
    ka¿dego cz³onka zespo³u nale¿a³o upewnienie siê, ¿e po zmianach przez niego wprowadzonych projekt uruchamia siê i nie
    pojawi³y siê nowe problemy (regresje). W przypadku, gdyby jednak regresje wyst±pi³y, mechanizmy kontroli wersji udostêpniane
    przez Gita pozwoli³yby na odtworzenie starszej wersji projektu.\\

    Czêste spotkania projektowe umo¿liwi³y nam pozostanie na bie¿±co z aktualnymi zmianami. W czasie takich sotkañ ka¿dy z cz³onków 
    zespo³u relacjonowa³ postêp prac, który nastêpnie by³ weryfikowany przez lidera projektu, b±d¼ te¿ przez innego cz³onka zespo³u. \\

\chapter{Wymagania projektowe}

\section{Rozpoznanie problemu}

    Sztuczna inteligencja jest ostatnio bardzo modn± dziedzin± informatyki. Znajduje zastosowania w kolejnych ga³êziach 
    informatyki i nie tylko. Nic wiêc dziwnego, ¿e coraz wiêcej ludzi wyra¿a zainteresowanie t± dziedzin± informatyki jednocze¶nie
    pragn±c siê nauczyæ czego¶ wiêcej. W tym momencie napotykamy problem. W internecie istnieje wiele platform pomagaj±cych nauczyæ
    siê algorytmiki (przede wszystkim popularne platformy przygotowuj±ce do konkursów ACM), jednak nie ma ¿adnego odpowiednika platformy 
    umo¿liwiaj±cej nauki sztucznej inteligencji.\\

    W odpowiedzi na tê potrzebê zdecydowali¶my siê stworzyæ serwis AI-Arena. Jest to serwis umo¿liwiaj±cy porównanie 
    programów walcz±cych. Program walcz±cy mo¿na opisaæ jako program graj±cy w pewn± grê, tj. pod±¿aj±cy wed³ug pewnych, ¶ci¶le
    okre¶lonych zasad. Zauwa¿my, i¿ wiêkszo¶æ algorytmów sztucznej inteligencji mo¿na z powodzeniem zastosowaæ w programach walcz±cych.\\

\section{Grupa docelowa}

    Jak wynika z dotychczasowego okre¶lenia problemu grupê docelow± stanowi± prorami¶ci, którzy s± zafascynowani tematem sztucznej inteligencji.
    Definiuje to w pewnym sensie sposób wykonania projektu. Mo¿na siê spodziewaæ, ¿e czê¶æ grupy, zafasynowana równie¿ bezpieczeñstwem
    systemów komputerowych, bêdzie chcia³a sprawdziæ zabezpieczenia systemu. Nale¿y wiêc ze zdwojonym wysi³kiem zapewniæ szeroko rozumiane
    bezpieczeñstwo.\\

    Z drugiej za¶ strony, poniewa¿ docelow± grupê u¿ytkowników stanowi± programi¶ci, mo¿emy nie przywi±zywaæ a¿ tak du¿ej wagi do graficznego
    interfejsu u¿ytkownika. Naturalnie interfejs powinien byæ wygodny, jednak programi¶ci s± raczej przyzwyczajeni do "nieprzyjaznego" wygl±du
    aplikacji, wobec  czego "surowy" wygl±d strony nie bêdzie na nich dzia³a³ odpychaj±co.\\
...
\begin{center}Top Coder\end{center} & algorytmy, problemy optymalizacyjne & okresowe & bardzo du¿a baza z przesz³ych zawodów \\ \hline
\begin{center}Ai Challenge\end{center} & programy walcz±ce & okresowe & konkurs co pó³ roku \\ \hline
\begin{center}SPOJ\end{center} & algorytmika & sta³e/okresowe & du¿a baza zadañ \\ \hline
\begin{center}AI-Arena\end{center} & programy walcz±ce & sta³e/okresowe & baza gier moderowana przez u¿ytkowników \\ \hline
\begin{center}SIO\end{center} & algorytmy & okresowe & wiêkszo¶æ zadañ dostêpna tylko podczas zawodów \\ \hline
\begin{center}MAIN\end{center} & algorytmy & sta³e & zadania z najwa¿niejszych zawodów informatycznych \\ \hline
\end{tabular}
\end{center}
...
Innym z zadañ nadzorcy jest odczytywanie informacji z stderr botów i sêdziego, a nastêpnie przekazanie ich warstwie schedulera. Pozwala to na umo¿liwienie u¿ytkownikowi na dostêp do informacji które jego program wypisywa³ na stderr. Nadzorca realizuje tê funkcjonalno¶æ, tworz±c dodatkowy w±tek odpowiedzialny za czytanie infromacji pojawiaj±cych siê na stderr sêdziego i botów. W±tek ten przy u¿yciu polecenia select czeka na mo¿liwo¶æ nieblokuj±cego odczytu na odpowiednich deskryptorach, a gdy taka mo¿liwo¶æ siê pojawia odczytuje informacje a¿ do opró¿nienia potoku zwi±zanego z danym deskryptorem.

Powa¿nym problemem jest wybór mechanizmu odpowiedzialnego za ograniczanie i zbieranie informacji o zu¿ywanych przez sêdziego i boty zasobach systemowych. Dostêpnych metod jest wiele, jednak nie wszystkie s± mo¿liwe do zastosowania w przypadku systemu ai-arena.

Ograniczenie zu¿ycia zasobów przez boty i sêdziego jest mo¿liwe do zrealizowania przez u¿ycie polecenia setrlimit. Polecenie to pozwala na ustawienie miêkkiego i twardego limitu na dostêpne procesowi zasoby systemowe, miêdzy innymi pamiêæ wirtualn± i czas zu¿ywany przez proces w trybie u¿ytkownika lub systemowym. 
...
Problem pomiaru zu¿ycia zasobów jest jednak nietrywialny. Dostêpne narzêdzia takie jak polecenie time lub skrypt timeout (projekt zapocz±tkowany na Institute for System Programming of
Russian Academy of Sciences, obecnie dostêpny jako open-source na github), nie mog± zostaæ u¿yte z tego samego powodu. Tworz± one dodatkowy proces, który uruchamia proces kontrolowany i mierzy jego zu¿ycie pamiêci. Oznacza to jednak ¿e tracimy mo¿liwo¶æ wysy³ania sygna³ów do botów i sêdziego, co jest potrzebne do zatrzymania lub zabicia ich procesów (np. gdy sêdzia za¿yczy sobie zabicie którego¶ z botów, lub gdy który¶ z programów pope³ni b³±d w komunikacji).

Rozwi±zaniem problemu jest samodzielne zaimplementowanie idei ze wspomnianego powy¿ej skryptu timeout. Polega ona na zrezygnowaniu z ustawienia limitów poleceniem setrlimit, sprawdzaniu co okre¶lony interwa³ czasu zasobów u¿ywanych przez kontrolowane procesy i zabiciu procesów gdy przekrocz± ustalone limity. Informacje o zasobach u¿ywanych przez proces o danym pid mo¿na uzyskaæ z katalogu /proc/[pid]/stats.
...
\begin{figure}[!h]
    \makebox[\textwidth]{
        \includegraphics[width=1.3\textwidth]{img/contests_uml.png}
    }
...
\begin{itemize}
    \item {\large Game} - zawiera plik z zasadami Gry oraz ¶cie¿kê do programu Sêdziego
    \item {\large Bot} - reprezentuje bota graj±cego w konkretn± Grê, zawiera m.in. plik ¼ród³owy
    \item {\large Contest} - zawiera informacje o Konkursie, a wiêc Grê, Ranking i listê Botów bior±cych w nim udzia³
    \item {\large Ranking} - od jego typu zale¿y w jaki sposób bêdzie rozgrywany Konkurs, np. "ka¿dy z ka¿dym"
    \item {\large BotRanking} - opisuje pozycjê Bota w danym Rankingu
    \item {\large Match} i {\large MatchBotResult} - przechowuj± informacjê o rozegranym Meczu tzn. które Boty bra³y w nim udzia³ oraz ich wyniki, wykorzystany czas i zu¿yt± pamiêæ
    \item {\large UserProfile} - najwa¿niejsze dane o u¿ytkowniku tj, zdjêcie, pañstwo pochodzenia
\end{itemize}
...
\begin{center}
\includegraphics[angle=90, width=1.0\textwidth]{img/UseCaseDiagram.jpg}
\end{center}
...
czyli osoby zainteresowane rozwijaniem go, mog± ³atwo uzyskaæ prawa do modyfikacji kodu.
...
\vspace{1cm}
Serwis mo¿e byæ wykorzystany równie¿ przez firmy szukaj±ce rozwi±zañ skomplikowanych problemów optymalizacyjnych. Wystarczy, ¿e
stworz± odpowiedni± grê, która mo¿e byæ równie¿ jednoosobowa i zorganizuj± konkurs dla u¿ytkowników serwisu. Przyk³adem platformy
...
Pracê nad projektem podzielili¶my na cztery kluczowe okresy:
...
\begin{itemize}
    ...
    \item{Dopracowywanie iterfejsu i ostatnie testy u¿ytkownika} - termin wykonania: \textbf{do 15 maja 2012 r.}
\end{itemize}

Zauwa¿my, ¿e taki plan prac umo¿liwia³, w wypadku nieporzewidzianych okoliczno¶ci, mo¿liwo¶æ zrealizowania 
ograniczonej funkcjonalno¶ci. Okaza³o siê jednak szczê¶liwie, i¿ opcja ta nie by³a potrzebna, gdy¿ postêpowali¶my zgodnie z harmonogramem.\\

\vspace{1cm}
W pierwszej fazie projektu skupili¶my siê na kluczowych zagadnieniach maj±cych wp³yw na ostateczny wygl±d serwisu. Powsta³y wtedy m.in. takie
...
W efekcie tych prac, przed koñcem listopada mogli¶my zaprezentowaæ dzia³aj±cy przyk³ad gry dwuosobowej nadzorowanej przed Nadzorcê. Gra ta
...
dzia³alno¶æ zwi±zana z u¿ytkowaniem serwisu. Da³o siê m.in. stworzyæ now± grê (maj±c zawczasu napisanego sêdziego), czy wys³aæ bota 
graj±cego w dan± grê (i realizuj±cego protokó³ okre¶lony w regu³ach gry).\\
...
\vspace{1cm}
Podczas trzeciej fazy projektu zosta³ znacznie rozwiniêty frontend. Zosta³y dodane funkcjonalno¶ci dodatkowe, zbêdne z punktu widzenia
...
W miêdzy czasie trwa³y prace równie¿ nad backendem, czyli warstw± Nadzorcy. Dodane zosta³y skrypty umo¿liwiaj±ce automatyczn± kompilacjê 
...
Udostêpniona zosta³a botom równie¿ mo¿liwo¶æ pisania logów na standardowy strumieñ b³êdów. Komunikaty wypisane na strumieñ b³êdów bêda przekazywane
botom po zakoñczeniu danej rozgrywki.\\

Oprócz postêpów nad frontendem i backendem, trzecia faza projektu zakoñczona zosta³a wro¿eniem systemu na udostêpniony go¶cinnie,
przez Codility, serwer. Zostali¶my przy tym poproszeni o przeprowadzenie w po³owie maja konkursu, pod patronatem Codility, w którym
platforma AI-Arena mia³aby odgrywaæ znacz±c± rolê.\\

Istotnym rezultatem tej czê¶ci projektu by³o równie¿ napisanie znacz±cej czê¶ci pracy licencjackiej. Wprowadzane pó¼niej zmiany ogranicza³y siê
do poprawienia literówek b±d¼ layoutu dokumentu.
...
\begin{itemize}
    \item{Rola w zespole} - Lider projektu
        \begin{itemize}
            ...
            \item{Prezentacja postêpów prac i planów na najbli¿szy okres, na spotkaniach z prowadz±cym zajêæ}
        \end{itemize}
    \item{Kod ¼ród³owy}
        \begin{itemize}
...
\begin{itemize}
    ...
    \item{Praca licencjacka}
        \begin{itemize}
            \item{Rozdzia³ 8: Dokumentacja u¿ytkowa i opis implementacji}
